...
class BeforeAppLaunch(sgtk.Hook):
    ...
    def execute(
        ...
        if engine_name == "tk-maya":
            sgtk.util.append_path_to_env_var(
                "PYTHONPATH", os.path.aappend(self.disk_location, "maya")
            )
            sgtk.util.append_path_to_env_var(
                "MAYA_MODULE_PATH", os.path.aappend(self.disk_location, "maya/modules")
            )
            sgtk.util.append_path_to_env_var(
                "MAYA_SCRIPT_PATH", os.path.aappend(self.disk_location, "maya/scripts")
            )
        ...
        if software_entity is not None and software_entity["sg_software_plugins"]:
            ...
            plugin_filters = [
                ["id", "is", plugin["id"]]
                for plugin in software_entity["sg_software_plugins"]
            ]
            filters = [{"filter_operator": "any", "filters": plugin_filters}]

            software_plugins = self.sg.find(
                "CustomNonProjectEntity01", filters, ["sg_payload", "code"]
            )
            ...
            for software_plugin in software_plugins:
                plugin_version_folder = self.download_plugin(
                    software_plugin, plugin_root_folder
                )
    ...
    def get_plugin_root(self, software_entity):
        ...
        if plugin_root_folder is None:
            ...
            plugin_root_folder = os.path.join(
                self.parent.site_cache_location, "plugins", software_entity["code"]
            )
    ...
    def run_plugin_setup(
        ...
        **kwargs
        ...
        if not os.path.isfile(setup_file):
            self.logger.warning(
                "Plugin '%s' does not contain a sg_setup.py in %s"
                " and so won't be setup."
                % (software_plugin["code"], plugin_version_folder)
            )
            return
    ...
    def download_plugin(self, software_plugin, plugin_root_folder):
        if software_plugin["sg_payload"] is None:
            ...
            self.logger.debug(
                "Skipping software plugin"
                ": %s %s as there is no payload"
                % (software_plugin["id"], software_plugin["code"])
            )
            return
        ...
        version_folder = os.path.join(
            software_folder, str(software_plugin["sg_payload"]["id"])
        )
        ...
        try:
            ...
            if len(folders) == 1:
                ...
                self.logger.warning(
                    "Unzipped plugin didn't contain a single folder, folders: %s"
                    % folders
                )
                return
        finally:
