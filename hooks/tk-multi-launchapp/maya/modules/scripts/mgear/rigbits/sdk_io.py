...
DESTINATION_TYPES = [SDK_UTILITY_TYPE[0], "transform", "blendShape", "joint", "mesh"]
...
def getSDKInfo(animNode):
    ...
    for index in range(0, numberOfKeys):
        ...
        absoluteValue = pm.keyframe(animNode, q=True, valueChange=True, index=index)[0]
...
def copySDKsToNode(sourceDriven, targetDriver, targetDriven, sourceAttributes=[]):
    ...
    for source, dest in sourceSDKInfo:
        ...
        sourceDriverAttr = pm.listConnections(
            "{0}.input".format(source.nodeName()), source=True, plugs=True, scn=True
        )[0]

        duplicateCurve = pm.duplicate(
            source,
            name="{0}_{1}".format(targetDriven.name(), dest.attrName(longName=True)),
        )[0]

        pm.connectAttr(
            "{0}.{1}".format(targetDriver, sourceDriverAttr.attrName(longName=True)),
            "{0}.input".format(duplicateCurve),
        )

        # drivenNode, drivenAttr = getSDKDestination(duplicateCurve.output)
        drivenAttrPlug = "{0}.{1}".format(targetDriven, dest.name(includeNode=False))
...
def mirrorSDKkeys(node, attributes=[], invertDriver=True, invertDriven=True):
    ...
    for source, dest in sourceSDKInfo:
        ...
        invertKeyValues(animCurve, invertDriver=invertDriver, invertDriven=invertDriven)
...
def createSDKFromDict(sdkInfo_dict):
    ...
    """
    sdkName = "{0}_{1}".format(sdkInfo_dict["drivenNode"], sdkInfo_dict["drivenAttr"])
    sdkNode = pm.createNode(sdkInfo_dict["type"], name=sdkName, ss=True)
    pm.connectAttr(
        "{0}.{1}".format(sdkInfo_dict["driverNode"], sdkInfo_dict["driverAttr"]),
