...
class CustomActions(HookBaseClass):
    ...
    def generate_actions(
        ...
        if file["type"] == "publish":
            return [{"name": "download_publish", "caption": "Download publish from mw_cloud"}]
        else:
            return []
        ...
        if action == "download_publish":
            ...
            if publishedFile != None:
                ...
                loader_app.execute_hook_expression("{config}/tk-multi-loader2/tk-maya_actions.py",
                                                   "_ensure_file_is_local", path=file["path"], published_file=publishedFile)
